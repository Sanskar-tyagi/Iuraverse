import { getAllUsers } from "@/hooks/useAddUser";
import {
  ColumnFiltersState,
  SortingState,
  VisibilityState,
...
const UserManagement = () => {
  ...
  const getUsers = getAllUsers().then((d) => {
  ...
  const [data, setNewCases] = useState(
    users?.filter((item: any) => item.isAccepted === false)
  );
  useEffect(() => {
    setNewCases(users?.filter((item: any) => item.isAccepted === false));
  }, [users]);
